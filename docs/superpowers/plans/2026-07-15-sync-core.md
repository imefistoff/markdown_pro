...
public struct SyncReplayer {
    ...
    private func applyOne(_ op: Op, adopted: Set<String>) throws {
        ...
        // entity is locally materialized or its project adopted: tombstone
...
```swift
    ...
    /// Runs a sync synchronously on the main actor. `Store` is `@MainActor` and
    /// the `Repository`/`SQLiteConnection` is single-threaded (FULLMUTEX
    /// serializes individual calls but NOT multi-statement transactions), so
    /// running on the main actor is the only race-free way to drive the engine
    /// without colliding with user edits (`perform`) or the 1.5 s poll. A brief
    /// UI hitch during a small local-folder sync is acceptable.
    func syncNow() {
        ...
        refresh()
        adoptable = (try? engine.availableToAdopt()) ?? []
    }
```

> Keep `private var isSyncing = false` as a re-entrancy guard (the debounced
> timer or quit path must not re-enter a running sync). Do NOT run the sync on a
> background thread with a second connection: both connections would issue HLC
> stamps under the same device id (user edits vs. `content_hash` ops during
> sync) from independent clocks, producing colliding stamps. One connection,
> one clock, main-actor-serialized is the correct model.
